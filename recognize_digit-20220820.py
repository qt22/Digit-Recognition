        ...
        for epoch in range(self.epochs):
            for label, image in zip(labels, images):
                ...
                for i in range(2, len(self.network)):
                    # need to loop through these layers later
                    layer = self.network[i]
                    ...
                    # need this in the dot product
                    prev_layer_values = self.get_layer_values(i-1)
                    
                    # each neuron has connection all the neurons from the previous layer
                    # to get all the weights (connections), we need to loop through both layers 
                    for p in range(len(prev_layer)):
                        ...
                        for n in range(len(layer)):
                            ...
                            # dot product + bias
                            temp_value = np.dot(prev_layer_values, neuron.weights) + neuron.bias
                            # weight to be used in the chain rule
                            weight = neuron.weights[p]
                            
                            # weight * f'(dot product + bias)
                ...
                ### compute partial derivatives for all weights and bias and apply Stochastic Gradient Descent
                
                # print(len(weights_partial_derivatives)) # 784 * 28 + 28 * 28 + 10 * 28 = 23016 weights
                # print(len(value_value_partial_derivatives)) # 2 * 28 * 28 = 1568
                # print(len(result_output_partial_derivatives)) # 10
                
                
                # calculate weight partial derivatives one layer at a time
                # last layer weights don't need value_value_partial_derivatives
                for i in range(1, len(self.network)): # 23016 weights in total for 2 hidden layers 
                    
                    layer = self.network[i]
                    next_layer = self.network[i+1] if (i+1) < len(self.network) else []
                    
                    # start with one neuron at a time
                    for n in range(len(layer)):
                        neuron = layer[n]
                        
                        # iterate through all weights it has
                        for w in range(len(neuron.weights)):
                            # for each weight, add up all the paths
                            # chain rule for partial derivatives
                            
                            end_partial_derivative = weights_partial_derivatives[i-1][n][w]
                            
                            # layer 2 -> layer 3: 28
                            # layer 3 -> layer 4: 28 * 10 = 280
                            # total paths count (i.e. size of list) = 28 * 280 = 7840 
                            middle_partial_derivatives = [] 
                            
                            # uhh needs to consider cases when the weight is in the last layer
                            # calculate partial derivatives
                            n_layer = self.network[i+1]
                            for nn in range(len(n_layer)):
                                n_neuron = n_layer[nn]
                                # the partial derivative connection has the same length 
                                for ww in range(len(n_neuron.weights)):
                                    neuron_partial_derivative = value_value_partial_derivatives[i-1][nn][ww]
                                    for j in range (i, len(value_value_partial_derivatives))
